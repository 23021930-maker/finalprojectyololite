...
protected:
    ...
    // buffer instances
    topp_f1_memcore* buffer[BufferCount];
...
public:
    ~topp_f1();
    SC_CTOR(topp_f1) : m_trace_file(0) {
        for (unsigned i = 0; i < BufferCount; i++) {
            char szTmp[128];
            sprintf(szTmp, "%d", i);
            std::string name = "topp_f1_memcore_U_";
            name += szTmp;

            buffer[i] = new topp_f1_memcore(name.c_str());
            buffer[i]->clk      ( clk );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "topp_f1.trace");
            // ports
