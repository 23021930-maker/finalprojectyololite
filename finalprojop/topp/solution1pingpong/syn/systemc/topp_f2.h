...
protected:
    ...
    // buffer instances
    topp_f2_memcore* buffer[BufferCount];
...
public:
    ~topp_f2();
    SC_CTOR(topp_f2) : m_trace_file(0) {
        for (unsigned i = 0; i < BufferCount; i++) {
            char szTmp[128];
            sprintf(szTmp, "%d", i);
            std::string name = "topp_f2_memcore_U_";
            name += szTmp;

            buffer[i] = new topp_f2_memcore(name.c_str());
            buffer[i]->clk      ( clk );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "topp_f2.trace");
            // ports
